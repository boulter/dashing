...
SCHEDULER.every '10m', :first_in => 0 do |job|
  ...
  path = "/forecastrss?w=#{woe_id}&u=#{format}&random=#{random}"
#  puts path
  ...
  weather_forecast = XmlSimple.xml_in(response.body, { 'ForceArray' => false })['channel']['item']['forecast'][0]
#  puts weather_forecast
  send_event('weather', { :temp => "#{weather_data['temp']}&deg;#{format.upcase}",
                          :condition => weather_data['text'],
			  :high => weather_forecast['high'],
			  :low => weather_forecast['low'],
